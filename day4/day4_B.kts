...
if (args.size != 1) {
    ...
    runCatching {
        ...
        val message = puzzleInput.check(::isOverlapping)
            .count { it == true }
...
fun isOverlapping(lists: List<List<Int>>) : Boolean = lists[0].intersect(lists[1]).size > 0 || lists[1].intersect(lists[0]).size > 0
